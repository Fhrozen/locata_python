...
class ProcessTask():
        ...
        for this_recording in recordings:
            ...
            for array_dir in array_names:
                ...
                if this_array not in self.arrays:
                    continue
                rec_id = [this_task, recording_id, this_array]
                self.log.info(f'Processing task {rec_id[0]}, recording {rec_id[1]}, array {rec_id[2]}.')
                # Load data
                ...
                if 'Forward' in self._task_process:
                    results = self.forward(
                        rec_id, audio_array, position_array, array_dir, required_time, groundtruth)
                else:
    ...
    def forward(self, rec_id, audio_array, position_array, array_dir, required_time, truth):
        ...
        if self._save_results:
            ...
            x_limit = x_axis.shape[0] / float(audio_array.fs)
            ...
            result_dir = os.path.join(result_dir, self.algorithm.__name__)
            ...
            for source_id in range(len(results.source)):
                ...
                if self._save_plots:
                    self.save_plots(
                        filename, x_axis, df[['azimuth']].values, df[['elevation']].values,
                        truth.source[_source_id].polar_pos, x_limit, rec_id)
        return results
    
    def save_plots(self, filename, wav_signal, azi, ele, truth, x_limit, rec_id):
        t_axis = np.linspace(0, x_limit, wav_signal.shape[0])
        azu_x_pd = np.degrees(azi)
        azu_x_gt = np.degrees(truth[:, 0])
        azu_t = np.linspace(0, x_limit, azu_x_gt.shape[0])

        ele_x_pd = np.degrees(ele)
        ele_x_gt = np.degrees(truth[:, 1])
        ele_t = np.linspace(0, x_limit, ele_x_gt.shape[0])

        fig, ax = plt.subplots(3, figsize=(4, 6))
        fig.tight_layout(pad=2.0)
        ax[0].plot(t_axis, wav_signal)
        ax[0].set_title(f'Task {rec_id[0]}, recording {rec_id[1]}, array: {rec_id[2]}')
